...
可以看到，分配时的表现符合我们的预期，而回收时只回收了一个进程的内存，这是由于一个进程运行到最后时进行了 Exit 或 Halt 调用，直接使得系统停机，因此之后的回收进程实际上并未进行。但经过测试我们已经可以看到我们的修改已经支持了多个线程同时存在与内存中，故仅对于该 Exercise 而言，我们已经完成任务。
